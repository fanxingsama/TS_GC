...
def remove_low_fluctuation_sensors(
    csv_path, 
    output_path, 
    fluctuation_threshold=50, 
    ignore_na=True,
    exclude_sensors=[]  # 新增：指定需要强制删除的测点列表
):
    """
    从时序CSV文件中删除波动次数极小的测点（列），同时删除.out后缀测点、同名测点中的SV后缀测点，以及指定强制删除的测点
    ...
    exclude_sensors: list - 需要强制删除的测点名称列表（如：["TIC_1101.SV", "PIC_2001.OUT"]），默认空列表
    """
    ...
    # ===== 新增步骤：删除指定的测点 =====
    # 先筛选出不在排除列表中的列
    exclude_sensors = [sensor.strip() for sensor in exclude_sensors]  # 去除首尾空格，避免输入误差
    specified_removed_columns = [col for col in df.columns if col in exclude_sensors]
    non_exclude_columns = [col for col in df.columns if col not in exclude_sensors]
    
    # 打印指定测点删除信息
    if specified_removed_columns:
        print(f"\n2. 检测到 {len(specified_removed_columns)} 个指定删除的测点，已删除：")
        for col in specified_removed_columns:
            print(f"  - {col}")
    else:
        print("\n2. 未检测到需要强制删除的指定测点")
    
    # 过滤指定删除的测点
    df = df[non_exclude_columns]
    print(f"   过滤指定删除测点后剩余 {df.shape[1]} 个测点")

    # ===== 第二步（原第三步）：删除同名测点中的SV后缀测点 =====
    # 构建测点名称映射：基名 -> [所有后缀列]
    sensor_base_map = {}
    ...
    if sv_removed_columns:
        print(f"\n3. 检测到 {len(sv_removed_columns)} 个SV后缀的重复测点，已删除：")  # 序号调整为3
        for col in sv_removed_columns:
            print(f"  - {col}")
    else:
        print("\n3. 未检测到需要删除的SV后缀重复测点")  # 序号调整为3
    ...
    print(f"删除指定强制删除测点：{len(specified_removed_columns)}")  # 新增统计项
...
if __name__ == "__main__":
    # 配置参数（按需修改！）
    INPUT_CSV_PATH = "主风机阀门全关.csv"  # 你的输入CSV文件路径
    OUTPUT_CSV_PATH = "主风机阀门全关_可用.csv"  # 输出文件路径
    FLUCTUATION_THRESHOLD = 30  # 波动次数阈值（比如你说的几十次，这里默认50）
    # 新增：指定需要强制删除的测点列表（填写你要删除的测点名称）
    EXCLUDE_SENSORS = [
        "DR1006.PV",  # 示例测点1
        "DR1005.PV",  # 示例测点2
        "DR1003.PV"       # 示例测点3（无后缀）
    ...
    remove_low_fluctuation_sensors(
        ...
        ignore_na=True,
        exclude_sensors=EXCLUDE_SENSORS  # 传入指定删除的测点列表
    )