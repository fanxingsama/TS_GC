...
def main(model, config, data_loader, gt, bigdata=False):
    ...
    # 从 config 中获取当前使用的数据集路径，并提取纯数据集名
    # 例如 "../fMRI/timeseries4.csv" -> "timeseries4"
    try:
        data_dir = config['data_loader']['args']['data_dir']
        dataset_name = os.path.basename(data_dir).replace('.csv', '')
    except Exception:
        dataset_name = "unknown" # 如果获取失败的保底命名
        
    # 确保当前路径下存在 csv 文件夹
    csv_dir = os.path.join(os.getcwd(), 'csv')
    os.makedirs(csv_dir, exist_ok=True)
    
    # 动态拼接文件名：CausalFormer_数据集名.csv (例如 CausalFormer_timeseries4.csv)
    csv_filename = os.path.join(csv_dir, f'CausalFormer_{dataset_name}.csv')
    ...
    with open(csv_filename, mode='w', newline='', encoding='utf-8') as csvfile:
        ...
        for e in ans:
            # 保持控制台的日志打印
            logger.info(f"{columns[e[0]]} causes {columns[e[1]]} with a delay of {e[2]} time steps.")
            # 写入 CSV：第一列是因 (columns[e[0]])，第二列是果 (columns[e[1]])，无表头
            writer.writerow([columns[e[0]], columns[e[1]]])
            
    logger.info(f"因果关系已成功保存至: {csv_filename}")
