...
# train_TS_GC.py 的开头修改这两个函数

def PGD_update(network, lam, lr, penalty, target_idx=None):
    # 【新增】保存自因果的权重，不让它被软阈值算子归零
    original_self_weights = network.data[:, target_idx, :].clone() if target_idx is not None else None
    ...
    # 【新增】恢复自因果权重，使其不受稀疏约束影响
    if target_idx is not None:
        network.data[:, target_idx, :] = original_self_weights


def lasso_penalty(network, lam, penalty, target_idx=None):
    # 【新增】计算Loss时，将自因果权重视为0，不计入惩罚项
    if target_idx is not None:
        network = network.clone()
        network[:, target_idx, :] = 0.0
...
class TS_GC_Trainer:
    ...
    def train(self, save_model=False):
        ...
        for ista_iter in range(self.epochs):
            ...
            if ista_iter >= warmup_epochs and self.lasso_param > 0:
                with torch.no_grad():
                    # 【修改这里】加入 enumerate 以获取目标序列索引 i
                    for i, weight_tensor in enumerate(self.first_layer_params_list): 
                        if weight_tensor is not None:
                             PGD_update(weight_tensor, self.lasso_param, self.base_lr, self.penalty_type, target_idx=i)
    ...
    def _compute_lasso_loss(self):
        ...
        for i, weight_tensor in enumerate(self.first_layer_params_list):
            if weight_tensor is not None:
                 total_lasso_loss += lasso_penalty(weight_tensor, self.lasso_param, self.penalty_type, target_idx=i)
