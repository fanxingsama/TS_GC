...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            # 将轮次和训练结果存储到日志字典中
            log = {'epoch': epoch, 'time': f"{epoch_time:.2f} seconds"}
            log.update(result)
            
            # 打印训练时间和损失
            self.logger.info(f"Epoch {epoch} completed in {epoch_time:.2f} seconds")
            self.logger.info(f"Training loss: {result['loss']:.6f}")
            if self.do_validation:
                self.logger.info(f"Validation loss: {result['val_loss']:.6f}")
    ...
    def _valid_epoch(self, epoch):
        ...
        self.logger.info(f'Average validation loss for epoch {epoch}: {avg_val_loss:.6f}')
