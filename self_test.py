...
def constrain_with_hard_threshold(gc_csv_path, output_csv_path, series_names, threshold=0.004):
    ...
    for target_idx in range(series_num):
        for source_idx in range(series_num):
            # 【删除 if source_idx != target_idx: 这层判断，减少缩进】
...
def main(model_path):
    ...
    # 【修改这里】ignore_self_causality=False
    res = save_gc_matrix_to_csv(model, gc_predict_path, SERIES_NAME, threshold=0.0, ignore_self_causality=False, is_softmax=False)
