...
def find_duplicate_sensor_columns(
    ...
    encoding='utf-8',
    na_values=['', 'NA', 'nan'],
    similarity_method='correlation',  # correlation/value/percent
    corr_threshold=0.99,              # 相关系数阈值（correlation模式）
    value_tolerance=1e-6,             # 数值误差容忍度（value模式）
    percent_threshold=99.0,           # 相同值占比阈值（percent模式）
    show_details=True                 # 是否显示详细的重复信息
):
    """
    识别CSV中测量同一测点的重复传感器列
    ...
    参数:
        ...
        encoding (str): 文件编码
        na_values (list): 缺失值标记
        similarity_method (str): 相似度判断方式
            - correlation: 皮尔逊相关系数（适合数值小幅波动的重复传感器）
            - value: 逐值完全匹配（允许微小数值误差）
            - percent: 相同值占比（适合部分数据不一致的重复传感器）
        corr_threshold (float): 相关系数阈值（0~1），越接近1越严格
        value_tolerance (float): 数值误差容忍度
        percent_threshold (float): 相同值占比阈值（%），0~100
        show_details (bool): 是否显示重复组的详细相似度信息
    """
    # 输入验证
    if not os.path.exists(input_file):
        raise FileNotFoundError(f"输入文件不存在: {input_file}")
    if similarity_method not in ['correlation', 'value', 'percent']:
        raise ValueError("相似度判断方式仅支持：correlation/value/percent")
    ...
    try:
        ...
        # 初始化重复组和已检查列（避免重复检测）
        checked_cols = set()  # 已检查过的列
        duplicate_groups = []  # 最终的重复组
...
if __name__ == "__main__":
    # ====================== 配置参数 ======================
    INPUT_CSV_PATH = "data.csv"  # 替换为你的CSV文件路径
    FILE_ENCODING = "utf-8"                     # 文件编码（如gbk）
    
    # 相似度判断配置（根据数据特性选择）
    SIMILARITY_METHOD = "correlation"  # correlation/value/percent
    CORR_THRESHOLD = 0.9999              # 相关系数阈值（建议0.95~1.0）
