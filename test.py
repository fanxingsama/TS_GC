...
def load_ground_truth_gc_matrix(csv_path, series_num):
    try:
...
def create_sequences_from_df(data_df, input_window, output_window, series_cols):
    X_list, Y_list = [], []
    data_np = data_df[series_cols].values.astype(np.float32) 
    ...
    if num_timesteps < input_window + output_window:
        ...
        return torch.empty(0, input_window, num_series, dtype=torch.float32), torch.empty(0, output_window, num_series, dtype=torch.float32)
...
def get_predictions_and_actuals(model, X_data, Y_data, device, max_samples=300):
    model.eval()
    predictions_list = []
    actuals_list = []
    ...
    with torch.no_grad():
        ...
        predictions_list.append(model_preds.cpu().numpy())
        actuals_list.append(Y_data_subset.cpu().numpy())
...
def plot_prediction_comparison_figure(predictions, actuals, series_num, output_window, model_path, num_series_to_plot=5, points_to_plot=300):
    if predictions.size == 0 or actuals.size == 0:
        print("没有预测数据或实际数据可供绘图。")
        return

    num_available_points = predictions.shape[0]
    ...
    for i, series_idx in enumerate(plot_indices):
        ax = axes[i]
        ax.plot(time_axis, actuals[:points_to_plot, output_step_idx, series_idx], label='实际值', color='blue')
        ax.plot(time_axis, predictions[:points_to_plot, output_step_idx, series_idx], label='预测值', color='red', linestyle='--')
...
def main(specified_run_id=None):
    ...
    # --- 1. 格兰杰因果矩阵对比 ---
    GC_true = load_ground_truth_gc_matrix(GROUND_TRUTH_GC_PATH, series_num)
    if GC_true is None:
    ...
    if not DATA_FOR_PREDICTION_PATH.exists():
        print(f"数据文件未找到: {DATA_FOR_PREDICTION_PATH}。跳过预测绘图。")
    else:
        prediction_input_df = pd.read_csv(DATA_FOR_PREDICTION_PATH)
    
        if not prediction_input_df.empty:
            if prediction_input_df.shape[1] < series_num:
                ...
                X_all_sequences, Y_all_sequences = create_sequences_from_df(prediction_input_df, input_window, output_window, all_series_cols)

                if X_all_sequences.shape[0] > 0:
                    predictions_np, actuals_np = get_predictions_and_actuals(model, X_all_sequences, Y_all_sequences, DEVICE, max_samples=MAX_PREDICTION_POINTS_TO_PLOT)
                    
                    if predictions_np.size > 0 :
                        plot_prediction_comparison_figure(predictions_np, actuals_np, series_num, output_window, model_path, 
             ...
             print(f"加载的数据文件 {DATA_FOR_PREDICTION_PATH} 为空，跳过预测绘图。")
