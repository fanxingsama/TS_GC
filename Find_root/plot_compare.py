...
rcParams['axes.titlesize'] = 24      # 子图标题字体放大
rcParams['axes.labelsize'] = 22      # 坐标轴标签字体放大
rcParams['xtick.labelsize'] = 22     # X轴刻度字体放大
rcParams['ytick.labelsize'] = 22     # Y轴刻度字体放大
# 图例字体稍微调小一点点（14），防止在子图里遮挡曲线
rcParams['legend.fontsize'] = 16     

def plot_comparisons_with_loaded_model(normal_csv_path, abnormal_csv_path, model_pkl_path, output_filename="Top10_Variables_10x3_Comparison.png"):
    # ==========================================
    # 1. 加载数据
    # ==========================================
    ...
    for row_idx, feat_idx in enumerate(top_indices):
        ...
        ax_norm.set_ylabel(f"{feat_name}", fontweight='bold', labelpad=10)
        ...
        if row_idx == 0:
            ax_spe.set_title("潜在异常变量异常分数", fontweight='bold', pad=20)

        # 绘制阈值红线
        var_thresh = model.var_SPE_thresholds[feat_idx]
        ax_spe.axhline(var_thresh, color='red', linestyle='--', linewidth=2.0, label=f'阈值: {var_thresh:.2f}')
...
if __name__ == "__main__":
    ...
    OUTPUT_IMAGE = "Top10_Variables_10x3_Comparison.png"
